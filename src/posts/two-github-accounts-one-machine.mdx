...
<Callout variant="warning" title="Secure your credentials">
  If the credential helper is set to `store`, then your tokens are being saved as plaintext at `~/.git-credentials` and you should fix that immediately - it's not secure.
</Callout>
...
<Callout variant="warning" title="Check if your office blocks SSH">
  It's common for corporate networks to block both incoming traffic on port 22 - the port that SSH listens on, as well as outgoing requests to port 22. GitHub itself offers SSH-over-443 (ssh.github.com) as a workaround.
</Callout>
...
<Callout variant="info" title="Use it or lose it">
  As an extra safety precaution - regardless of the expiration date you set - GitHub will automatically delete any token or SSH key that haven't been used in a year.
</Callout>
...
<Callout variant="warning" title="Edge cases">
  There's one drawback worth noting about the `insteadOf` method - if I ever needed to clone one of my own repos using my work account (say I was added as a collaborator under my work identity), this wouldn't be possible with the current setup and I'd have to come up with another workaround (add a third alias maybe).
</Callout>
